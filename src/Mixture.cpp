...
{
    double probability = (1.0 / sqrt(2.0 * M_PI * variance)) * (exp(-pow(intensity - mean, 2.0) / (2.0 * variance)));
    return probability;
}
...
void Mixture::updateMixture(double intensity)
{
    ...
    for (int n = 0; n < this->numberOfGaussians; n++)
    {
        // if |I_{t,x} - mu_{t-1,x,n}| <= 2.5 * sigma_{t-1,x,n} then d_{t,x,n} = -w_{t-1,x,n}
    ...
    int currentGaussianIndex = distance(distances.begin(), min_element(distances.begin(), distances.end()));
    ...
    // Model renewing
    // w_{t,x,n} = (1 - eta_{t,x}(Beta)) * w_{t-1,x,n} + eta_{t,x}(Beta) * M_{t,x,n}, for some n
    for (int n = 0; n < this->numberOfGaussians; n++)
    {
        this->gaussians[n].setWeight((1.0 - this->eta) * this->gaussians[n].getWeight() + this->eta * static_cast<double>(modelMatching[n]));
    }
    ...
    {
        // Update phase
        // rho_{t,x,l(t,x)}(alpha) = alpha * GF(I_{t,x}, mu_{t-1,x,l(t,x)}, sigma^2_{t-1,x,l(t,x)})
        double palpha = this->alpha * calculateProbability(intensity, this->gaussians[currentGaussianIndex].getMean(), this->gaussians[currentGaussianIndex].getVariance());
        // mu_{t,x,l(t,x)} = (1 - rho_{t,x,l(t,x)}(alpha)) * mu_{t-1,x,l(t,x)} + rho_{t,x,l(t,x)}(alpha) * I_{t,x}
        this->gaussians[currentGaussianIndex].setMean((1.0 - palpha) * this->gaussians[currentGaussianIndex].getMean() + palpha * intensity);
        // sigma^2_{t,x,l(t,x)} = (1 - rho_{t,x,l(t,x)}(alpha)) * sigma^2_{t-1,x,l(t,x)} + rho_{t,x,l(t,x)}(alpha) * (I_{t,x} - mu_{t,x,l(t,x)})^2
        this->gaussians[currentGaussianIndex].setVariance((1.0 - palpha) * this->gaussians[currentGaussianIndex].getVariance() + palpha * pow(intensity - this->gaussians[currentGaussianIndex].getMean(), 2.0));
    }
    else
    {
        // Replacement phase
        // k = argmin_{n=1,...,N} w_{t-1,x,n}
        int gaussianToReplaceIndex = distance(this->gaussians.begin(), min_element(this->gaussians.begin(), this->gaussians.end(), [](const Gaussian &g1, const Gaussian &g2)
        ...
        // w_{t,x,k} = w_0
        this->gaussians[gaussianToReplaceIndex].setWeight(1.0 / (double)this->numberOfGaussians);
    }
...
void Mixture::updateEta(int O, double intensity)
{
    // eta_{t,x}(Beta) =
    switch (O)
    {
    ...
    case 1:
    {
        ...
        this->eta = this->beta_d * calculateProbability(intensity, this->gaussians[currentGaussianIndex].getMean(), this->gaussians[currentGaussianIndex].getVariance());
        // Keep below Beta_b
